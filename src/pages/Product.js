...
function Product() {
  ...
  return (
    <main className='w-full h-full flex justify-center items-center p-20'> 
    <div className='h-full flex flex-row gap-4 flex-wrap items-center justify-center pt-4  '> 
      {(productName?.CategoriesArray)?.map((items) => {
         return(
          <Link to={`/${CatName}/${tagCodes }/${items?.tagCodes[0]}`}>
              <div className='w-[200px] h-[50px] rounded-sm bg-gray-300 flex justify-center items-center  hover:bg-red-700 hover:text-white hover:border-white'>
                {items.CatName}
              </div>
         </Link>
         )
